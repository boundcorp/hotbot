...
class CastAdmin(admin.ModelAdmin):
    ...
    def cast(self, obj):
        ...
        return format_html(
            '<div style="max-width: 400px;">{}<br />{}</div>',
            summary,
            obj.embed_descriptions
            and "Embeds: " + ", ".join(obj.embed_descriptions.values())
            or "",
        )

    def tags(self, obj):
        return format_html(
            ", ".join([tag.tag for tag in obj.cast_tags.all()])
            + "<br />"
            + (obj.moderation_analysis or {}).get("analysis", "")
        )
    ...
    def link(self, obj):
        if not obj.author:
            return None
        return format_html(
            '<a href="https://warpcast.com/{}/{}" target="_blank">{}</a>',
            obj.author.username,
            obj.hash,
            obj.hash[:10],
        )
    ...
    def log(self, obj):
        ...
        def log_line(log):
            duration = (
                datetime.fromisoformat(log["timestamp"]) - start
            ).total_seconds()
