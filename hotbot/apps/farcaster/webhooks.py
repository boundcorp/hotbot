...
def neynar_webhook_receiver(request):
    if request.method == "POST":
        ...
        logger.debug(
            f"received {kind} in {channel} by {body_data['data']['author']['username']}"
        )
...
def automod_webhook_exclude(request, channel_id):
    if request.method == "POST":
        ...
        if channel.moderation_enabled:
            logger.info(
                f"checking exclusion for {channel_id} by {body_data['user']['username']}"
            )
            result = automod_classify(body_data, channel)
            if result:
                if result.should_exclude:
                    logger.info(
                        f"excluding {channel_id} by {body_data['user']['username']}"
                    )
                    return trigger_rule(result.analysis)
                else:
                    logger.info(
                        f"no action for {channel_id} by {body_data['user']['username']}"
                    )
